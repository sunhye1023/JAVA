...
//		System.out.println(token1.nextToken()); // 날씨는
//		System.out.println(token1.nextToken()); // 맑고,
//		System.out.println(token1.nextToken()); // 매우
//		System.out.println(token1.nextToken()); // 시원합니다
		
		
		System.out.println(token1.countTokens()); // 토큰사용전에 몇 개의 토큰이 있는지 검사
		
		while(token1.hasMoreTokens()) { // 검사할 다음 토큰이 있으면 true반환
			System.out.println(token1.nextToken());
		}
		
		//System.out.println(token1.countTokens()); //더이상 남은 토큰이 없다
